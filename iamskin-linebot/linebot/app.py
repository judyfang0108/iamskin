...
def handle_message(event):
    ...
    if re.match("AI檢測", msg):
        FlexMessage = json.load(open('./flexMessage/AI_detect_menu.json','r',encoding='utf-8'))
...
def handle_img(event):
    ...
    if user_state.get(uid) == 1: # 膚質
        ...
        file_path = f"./Image/skin/{reply_token}.jpg"
        ...
        result_path = f"./Prediction/skin/{reply_token}_prediction.jpg"
        ...
        if list(res['prediction'].keys())[0] == 'normalnail':
            FlexMessage = json.load(open('./flexMessage/nail_result_low.json','r',encoding='utf-8'))
            line_bot_api.reply_message(event.reply_token, FlexSendMessage('指甲異常風險-低', FlexMessage))
        else:
            FlexMessage = json.load(open('./flexMessage/nail_result_high.json','r',encoding='utf-8'))
        ...
        elif res['prediction'] == 'Moderate': # middle
        ...
        elif res['prediction'] == "Severe": # m_high
